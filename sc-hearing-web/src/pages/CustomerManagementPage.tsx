...
function fileIcon(t:string,n:string):string{
  const e=n.split('.').pop()?.toLowerCase()??''
  if(t.includes('pdf')||e==='pdf')return'📄'
  if(t.includes('word')||['docx','doc'].includes(e))return'📝'
  if(t.includes('excel')||['xlsx','xls'].includes(e))return'📊'
  if(t.includes('powerpoint')||['pptx','ppt'].includes(e))return'📋'
  if(t.includes('image')||['jpg','jpeg','png','gif','webp'].includes(e))return'🖼️'
  return'📎'
}
...
const EMPTY_CUSTOMER: CustomerDto={name:'',industry:'',primeType:'プライム',partner:null,modules:'SC販売',version:null,proposalStatus:'',scMaintDate:null,serverEnv:null,serverMaintDate:null,contact:'永田 暁洋',customerContact:null,notes:null,monthlyFee:null}
...
function getUrgency(s:string|null):Urgency{
  if(!s)return'none';const d=parseDate(s);if(!d)return'none'
  const m=(d.getTime()-TODAY.getTime())/(1000*60*60*24*30)
  if(m<0)return'expired';if(m<6)return'critical';if(m<12)return'warning';if(m<24)return'caution';return'ok'
}
function cardUrgency(c:Customer):Urgency{
  const order:Urgency[]=['expired','critical','warning','caution','ok','none']
  const si=order.indexOf(getUrgency(c.serverMaintDate)),sci=order.indexOf(getUrgency(c.scMaintDate))
  return order[Math.min(si<0?99:si,sci<0?99:sci)]??'none'
}
function dateLabel(s:string|null,u:Urgency):string{
  if(!s)return'―';const d=parseDate(s);if(!d)return s
  const m=Math.round((d.getTime()-TODAY.getTime())/(1000*60*60*24*30))
  return u==='expired'?`${s}（期限切れ）`:`${s}（あと${m}ヶ月）`
}
...
export default function CustomerManagementPage(){
  ...
  const filtered=useMemo(()=>{
    ...
    return customers.filter(c=>{
      ...
      return true
    }).sort((a,b)=>ord.indexOf(cardUrgency(a))-ord.indexOf(cardUrgency(b)))
  },[customers,search,fContact,fUrgency,fType])
    ...
    <Layout>
      <div style={{maxWidth:1400,margin:'0 auto',padding:'1.5rem 2rem',fontFamily:'"Noto Sans JP",sans-serif'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
          <div>
            <h2 style={{margin:0,fontSize:'1.2rem',fontWeight:700,color:'#0f172a'}}>顧客管理</h2>
            <p style={{margin:'3px 0 0',fontSize:'0.78rem',color:'#64748b'}}>保守期限・インフラ・案件・添付資料を一元管理</p>
          </div>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            ...
            <button onClick={()=>setIsCreating(true)} style={{padding:'8px 20px',background:'#0f172a',border:'none',borderRadius:8,color:'white',fontWeight:700,fontSize:'0.85rem',cursor:'pointer'}}>＋ 顧客を登録</button>
          </div>
        </div>

        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:'1.25rem'}}>
          {[{label:'総顧客数',value:`${customers.length}`,unit:'社',color:'#0f172a'},{label:'要対応',value:`${criticalCount}`,unit:'社',color:'#dc2626'},{label:'進行中案件',value:`${activeCount}`,unit:'件',color:'#7c3aed'},{label:'月次保守料合計',value:totalFee?`¥${Math.round(totalFee/10000)}万`:'―',unit:'',color:'#059669'}].map(s=>(
            <div key={s.label} style={{background:'white',borderRadius:10,border:'1px solid #e2e8f0',padding:'14px 16px'}}>
              <div style={{fontSize:'0.7rem',color:'#94a3b8',marginBottom:4}}>{s.label}</div>
              <div style={{fontSize:'1.4rem',fontWeight:700,color:s.color}}>{s.value}<span style={{fontSize:'0.8rem',fontWeight:500,marginLeft:2}}>{s.unit}</span></div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:'flex',gap:8,marginBottom:'1rem',flexWrap:'wrap'}}>
          ...
          <select style={sel} value={fUrgency} onChange={e=>setFUrgency(e.target.value)}><option value="all">全ステータス</option><option value="expired">期限切れ</option><option value="critical">要対応（6ヶ月以内）</option><option value="warning">注意（1年以内）</option><option value="ok">正常</option></select>
          <select style={sel} value={fType} onChange={e=>setFType(e.target.value)}><option value="all">全種別</option><option value="プライム">プライム</option><option value="サブ">サブ</option></select>
        </div>

        {loading?<div style={{textAlign:'center',padding:'4rem',color:'#94a3b8'}}>読み込み中...</div>
          :viewMode==='gantt'?<CustomerGanttView customers={filtered} projects={projects} onProjectChange={loadProjects}/>
          :(
      ...
      {delTarget&&<ConfirmModal title="顧客を削除しますか？" message={`「${delTarget.name}」を削除します。\n関連する案件・添付ファイルも全て削除されます。\nこの操作は元に戻せません。`} onCancel={()=>setDelTarget(null)} onConfirm={async()=>{await customersApi.delete(delTarget.id);await load();setDelTarget(null)}}/>}
    </Layout>
  )
}

// ─── 顧客カード ───────────────────────────────────────────────
function CustomerCard({customer:c,projects,files,expanded,currentUser,onToggle,onEdit,onDelete,onAddProject,onEditProject,onDeleteProject,onFilesChange}:{
  customer:Customer;projects:CustomerProject[];files:CustomerFileItem[];expanded:boolean;currentUser:string
  ...
  return(
    <div style={{borderRadius:12,background:'white',border:'1px solid #e2e8f0',overflow:'hidden',boxShadow:expanded?'0 2px 16px rgba(0,0,0,0.06)':'none',transition:'box-shadow .2s'}}>
      {/* ── ヘッダー行 */}
      <div style={{display:'flex',alignItems:'stretch'}}>
        <div style={{width:4,background:uc.bar,flexShrink:0}}/>
        <div onClick={onToggle} style={{flex:1,minWidth:0,padding:'11px 14px',display:'flex',alignItems:'center',gap:12,cursor:'pointer'}}>
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:'0.72rem',color:'#94a3b8',marginTop:1}}>
              ...
              <span>担当 {c.contact}</span>
              {c.customerContact&&<span style={{marginLeft:7}}>/ 顧客 {c.customerContact}</span>}
            </div>
          </div>
          <div style={{display:'flex',gap:4,flexWrap:'wrap',alignItems:'center',flexShrink:0}}>
            ...
            {totalFileCount>0&&<span style={{fontSize:'0.65rem',padding:'2px 7px',borderRadius:4,background:'#f0f9ff',color:'#0369a1',fontWeight:700}}>資料{totalFileCount}</span>}
          </div>
      ...
      {expanded&&(
        <div style={{borderTop:'1px solid #f1f5f9',display:'flex',minHeight:0}}>
          ...
          <div style={{width:240,flexShrink:0,borderRight:'1px solid #f1f5f9',background:'#fafafa',padding:'12px 14px',fontSize:'0.78rem'}}>
            ...
            <CompactSection label="担当者・契約" color="#059669">
              <CKV label="TCS担当" value={c.contact}/>
              {c.customerContact&&<CKV label="顧客担当" value={c.customerContact}/>}
          ...
          <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column'}}>
            ...
            <div style={{display:'flex',borderBottom:'1px solid #f1f5f9',background:'#fafafa',flexShrink:0}}>
              {[
                {key:'projects' as const,label:`案件・プロジェクト`,count:projects.length,activeCount:active.length},
                {key:'files' as const,label:`顧客添付資料`,count:customerFiles.length,activeCount:0},
              ].map(t=>(
                <button key={t.key} onClick={()=>setActiveTab(t.key)}
                  style={{padding:'8px 16px',border:'none',borderBottom:activeTab===t.key?'2px solid #0f172a':'2px solid transparent',background:'transparent',color:activeTab===t.key?'#0f172a':'#64748b',fontWeight:activeTab===t.key?700:500,fontSize:'0.78rem',cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:5}}>
                  {t.label}
                  ...
                  {t.activeCount>0&&<span style={{fontSize:'0.63rem',padding:'0 5px',borderRadius:99,background:'#f5f3ff',color:'#7c3aed',fontWeight:700}}>{t.activeCount}進行中</span>}
                </button>
              ))}
              <div style={{flex:1}}/>
              {activeTab==='projects'&&(
                <button onClick={onAddProject} style={{margin:'6px 10px',padding:'4px 12px',border:'1px solid #e2e8f0',borderRadius:6,background:'white',color:'#475569',cursor:'pointer',fontSize:'0.72rem',fontWeight:600,outline:'none',fontFamily:'inherit'}}>＋ 案件を追加</button>
              )}
            </div>
            ...
            <div style={{flex:1,overflowY:'auto',maxHeight:360}}>
              ...
              {activeTab==='projects'&&(
                <div style={{padding:'8px 12px'}}>
                  {projects.length===0?(
                    <div style={{textAlign:'center',padding:'24px',color:'#94a3b8',fontSize:'0.8rem'}}>案件が登録されていません</div>
                  ):(
                    <>
                      {/* ステータスサマリ */}
                      {projects.length>0&&(
                        <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:8}}>
                          {['提案中','商談中','受注','対応中','完了','失注'].map(s=>{const cnt=projects.filter(p=>p.status===s).length;if(!cnt)return null;const st=STATUS_STYLE[s];return<span key={s} style={{fontSize:'0.63rem',padding:'1px 7px',borderRadius:99,background:st.bg,color:st.color,border:`1px solid ${st.border}`,fontWeight:700}}>{s} {cnt}</span>})}
                        </div>
                      )}
                      {/* 案件行 */}
                      <div style={{display:'flex',flexDirection:'column',gap:4}}>
                        {projects.map(p=>(
                          <ProjectRow key={p.id} project={p} customerId={c.id} allFiles={files}
                            currentUser={currentUser}
                            onEdit={()=>onEditProject(p)} onDelete={()=>onDeleteProject(p.id)}
              ...
              {activeTab==='files'&&(
                <div style={{padding:'8px 12px'}}>
                  <CompactFileSection customerId={c.id} projectId={null} allFiles={files} currentUser={currentUser} onFilesChange={onFilesChange}/>
                </div>
              )}
            </div>
          </div>
...
// ─── コンパクト情報セクション ─────────────────────────────────
function CompactSection({label,color,children}:{label:string;color:string;children:React.ReactNode}){
  return(
    <div style={{marginBottom:10}}>
      <div style={{display:'flex',alignItems:'center',gap:4,marginBottom:5}}>
        <div style={{width:3,height:10,borderRadius:2,background:color}}/>
        <span style={{fontSize:'0.63rem',fontWeight:700,color:'#64748b',letterSpacing:'0.05em',textTransform:'uppercase' as const}}>{label}</span>
      </div>
      <div style={{paddingLeft:7,display:'flex',flexDirection:'column',gap:3}}>{children}</div>
    </div>
  )
}
function CKV({label,value,vc}:{label:string;value:string;vc?:string}){
  return(
    <div style={{display:'flex',gap:6,alignItems:'baseline'}}>
      <span style={{fontSize:'0.65rem',color:'#94a3b8',flexShrink:0,width:52}}>{label}</span>
      <span style={{fontSize:'0.72rem',color:vc||'#334155',fontWeight:vc?600:400,lineHeight:1.3,wordBreak:'break-all'}}>{value}</span>
    </div>
  )
}

// ─── 案件行（コンパクト＋ファイル展開） ──────────────────────
function ProjectRow({project:p,customerId,allFiles,currentUser,onEdit,onDelete,onFilesChange}:{
  project:CustomerProject;customerId:number;allFiles:CustomerFileItem[];currentUser:string
  onEdit:()=>void;onDelete:()=>void;onFilesChange:()=>void
}){
  ...
  return(
    <div style={{border:'1px solid #e2e8f0',borderRadius:8,overflow:'hidden',background:isActive?'white':'#fafafa',opacity:isActive?1:0.75}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px'}}>
        ...
        <span style={{fontSize:'0.78rem',fontWeight:600,color:'#0f172a',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.projectName}</span>
        ...
        <button onClick={()=>setShowFiles(v=>!v)}
          style={{display:'flex',alignItems:'center',gap:2,padding:'2px 7px',border:'1px solid #e2e8f0',borderRadius:4,background:showFiles?'#f0f9ff':'white',color:showFiles?'#0369a1':'#64748b',cursor:'pointer',fontSize:'0.65rem',fontWeight:600,outline:'none',fontFamily:'inherit',flexShrink:0}}>
          📎{projectFiles.length>0&&<span style={{background:'#0369a1',color:'white',borderRadius:99,padding:'0 3px',fontSize:'0.58rem'}}>{projectFiles.length}</span>}
        </button>
      ...
      {/* 案件ファイルセクション */}
      {showFiles&&(
        <div style={{borderTop:'1px solid #f1f5f9',padding:'8px 10px 8px 28px',background:'#f8fafc'}}>
          <CompactFileSection customerId={customerId} projectId={p.id} allFiles={allFiles} currentUser={currentUser} onFilesChange={onFilesChange}/>
        </div>
      )}
...
// ─── コンパクトファイルセクション ────────────────────────────
function CompactFileSection({customerId,projectId,allFiles,currentUser,onFilesChange}:{
  customerId:number;projectId:number|null;allFiles:CustomerFileItem[];currentUser:string;onFilesChange:()=>void
}){
  ...
  const handleFiles=async(fl:FileList|null)=>{
    ...
    try{for(const f of Array.from(fl))await customerFilesApi.upload(customerId,f,undefined,currentUser,projectId??undefined);onFilesChange()}
  ...
  return(
    <div>
      {/* 小さいアップロードボタン＋ドロップエリア */}
      <div
        onDragOver={e=>{e.preventDefault();setDragOver(true)}}
        onDragLeave={()=>setDragOver(false)}
        onDrop={e=>{e.preventDefault();setDragOver(false);handleFiles(e.dataTransfer.files)}}
        style={{display:'flex',alignItems:'center',gap:8,marginBottom:displayFiles.length>0?6:0}}>
        <input ref={fileInputRef} type="file" multiple style={{display:'none'}} onChange={e=>handleFiles(e.target.files)}/>
        <button onClick={()=>fileInputRef.current?.click()} disabled={uploading}
          style={{display:'flex',alignItems:'center',gap:4,padding:'4px 10px',border:`1px dashed ${dragOver?'#0369a1':'#d1d5db'}`,borderRadius:6,background:dragOver?'#f0f9ff':'white',color:dragOver?'#0369a1':'#64748b',cursor:'pointer',fontSize:'0.72rem',fontWeight:600,outline:'none',fontFamily:'inherit'}}>
          {uploading?'アップロード中...':'📎 ファイルを追加'}
        </button>
        <span style={{fontSize:'0.62rem',color:'#94a3b8'}}>またはここにドロップ（最大20MB）</span>
      </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:3}}>
          {displayFiles.map(f=>(
            <div key={f.id} style={{display:'flex',alignItems:'center',gap:7,padding:'5px 8px',background:'white',borderRadius:6,border:'1px solid #e2e8f0'}}>
              ...
              <a href={customerFilesApi.getDownloadUrl(f.id)} target="_blank" rel="noopener noreferrer"
                style={{flex:1,minWidth:0,fontSize:'0.74rem',fontWeight:600,color:'#0369a1',textDecoration:'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}
              ...
              <a href={customerFilesApi.getDownloadUrl(f.id)} target="_blank" rel="noopener noreferrer"
                style={{width:22,height:22,border:'1px solid #bae6fd',borderRadius:4,background:'#f0f9ff',color:'#0369a1',textDecoration:'none',fontSize:'0.68rem',fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}} title="開く">↗</a>
              <a href={customerFilesApi.getDownloadUrl(f.id)} download={f.fileName}
                style={{width:22,height:22,border:'1px solid #d1fae5',borderRadius:4,background:'#f0fdf4',color:'#059669',textDecoration:'none',fontSize:'0.68rem',fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}} title="ダウンロード">↓</a>
              <button onClick={()=>setDelTarget(f)}
                style={{width:22,height:22,border:'1px solid #fecaca',borderRadius:4,background:'#fef2f2',color:'#dc2626',cursor:'pointer',fontSize:'0.62rem',outline:'none',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}} title="削除">✕</button>
            </div>
          ))}
        </div>
      )}
...
function CustomerFormModal({customer,onClose,onSave}:{customer:Customer|null;onClose:()=>void;onSave:(dto:CustomerDto)=>Promise<void>}){
  const[form,setForm]=useState<CustomerDto>(customer?{name:customer.name,industry:customer.industry,primeType:customer.primeType,partner:customer.partner,modules:customer.modules,version:customer.version,proposalStatus:'',scMaintDate:customer.scMaintDate,serverEnv:customer.serverEnv,serverMaintDate:customer.serverMaintDate,contact:customer.contact,customerContact:customer.customerContact,notes:customer.notes,monthlyFee:customer.monthlyFee}:{...EMPTY_CUSTOMER})
  ...
  return(
    <Modal title={customer?'顧客情報を編集':'新規顧客を登録'} onClose={onClose} onSave={handleSave} saving={saving} error={error}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 1rem'}}>
        ...
        <div style={fld}><label style={lbl}>パートナー</label><input style={inp} value={form.partner??''} onChange={e=>set('partner',e.target.value)} placeholder="サブの場合"/></div>
      ...
      <div style={{borderTop:'1px solid #f1f5f9',paddingTop:'0.85rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 1rem'}}>
          <div style={fld}><label style={lbl}>TCS担当者</label><select style={inp} value={form.contact} onChange={e=>set('contact',e.target.value)}>{CONTACTS.map(c=><option key={c}>{c}</option>)}</select></div>
          <div style={fld}><label style={lbl}>顧客担当者</label><input style={inp} value={form.customerContact??''} onChange={e=>set('customerContact',e.target.value)} placeholder="例：濱様"/></div>
        </div>
...
function ProjectFormModal({project,customerId,onClose,onSave}:{project:CustomerProject|null;customerId:number;onClose:()=>void;onSave:(dto:CustomerProjectDto)=>Promise<void>}){
  ...
  const handleSave=async()=>{if(!form.projectName.trim()){setError('案件名は必須です');return};setSaving(true);try{await onSave(form)}catch{setError('保存に失敗しました');setSaving(false)}}
  const inp:React.CSSProperties={width:'100%',padding:'8px 10px',border:'1px solid #e2e8f0',borderRadius:7,fontSize:'0.85rem',color:'#0f172a',background:'white',outline:'none',boxSizing:'border-box',fontFamily:'inherit'}
  const lbl:React.CSSProperties={display:'block',fontSize:'0.72rem',fontWeight:700,color:'#64748b',marginBottom:4}
  const fld:React.CSSProperties={marginBottom:'0.85rem'}
  return(
    <Modal title={project?'案件を編集':'案件を追加'} onClose={onClose} onSave={handleSave} saving={saving} error={error} maxWidth={480}>
